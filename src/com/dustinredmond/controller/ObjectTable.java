 ...
 */
public class ObjectTable<T> extends TableView<T> {

    /**
     * Constructs an {@code ObjectTable} suitable for displaying objects of
     * the specified model class.
     * @param modelClass Class whose fields are to be used as {@code TableColumn}s
     */
    public ObjectTable(Class<T> modelClass) {
    ...
    /**
     * Constructs an {@code ObjectTable} suitable for displaying objects of the
     ...
     * Use {@code new ObjectTable(Class clazz)} instead.
     */
    public ObjectTable(ObservableList<T> objects) throws UnsupportedOperationException {
        if (objects == null || objects.size() == 0) {
            throw new UnsupportedOperationException("The object list supplied when instantiating " +
                    "com.jfxdev.ObjectTable must not be null or size zero. Other constructors support this.");
        }
    ...
    /**
     * Renames a single table column. {@code ObjectTable().applyColumnNameMapping()}
